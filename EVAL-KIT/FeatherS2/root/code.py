...
def wifiInit():
    ...
    try:
        if config["mode"] == "sta":
            ...
            if config["ssid"] == "swarm":
                config["ssid"] = "swarm-" + "%02x%02x" % (
                    wifi.radio.mac_address[4],
                    wifi.radio.mac_address[5],
                )
...
def tileInit():
    global tile
    tile = busio.UART(
        board.TX, board.RX, baudrate=115200, receiver_buffer_size=8192, timeout=0.0
    )
    tileStart()
...
def inaPoll():
    ...
    if time.time() - inaTime > 5:
        try:
            ...
            displayLine(
                1, "%s %6.3fV %6.3fA" % (inaChans[inaChannel], bus_voltage, current)
            )
...
def tcpPoll():
    if config["wifi"] == "disabled" or (
        wifi.radio.ipv4_address_ap is None and wifi.radio.ipv4_address is None
    ):
        return
                            ...
                            if params[0] == "@reset":
                                ...
                                if params[1] == "mode":
                                    if params[2] in ["ap", "sta"]:
                                        ...
                                        tcpconn.send(
                                            f"Successfully set mode to {params[2]}."
                                        )
                                        writePreferences()
                                if params[1] == "wifi":
                                ...
                                if params[1] == "ssid":
                                    ...
                                    tcpconn.send(
                                        f"Successfully set ssid to {config['ssid']}."
                                    )
                                    writePreferences()
                                if params[1] == "pw":
                                    config["password"] = command[8:].strip()
                                    tcpconn.send(
                                        f"Successfully set password to {config['password']}."
                                    )
                                    writePreferences()
                                if params[1] == "interval":
                                    if int(params[2]) == 0 or (
                                        int(params[2]) >= 15 and int(params[2]) <= 720
                                    ):
                                        if (
                                            int(params[2]) == 0
                                            and config["interval"] > 0
                                        ):
                                            config["interval"] = config["interval"] * -1
                                            tcpconn.send(
                                                f"Successfully set interval to off."
                                            )
                                        else:
                                            config["interval"] = int(params[2])
                                            tcpconn.send(
                                                f"Successfully set interval to {config['interval']}."
                                            )
                                        gpsInit()
                                        writePreferences()
                                    else:
                                        tcpconn.send(
                                            "Interval can only be 0 or 15-720 minutes."
                                        )
                            elif params[0] == "@show":
                                if len(params) == 2:
                                    if params[1] == "battery":
                                    ...
                                    tcpconn.send(
                                        "wifi pw:  " + config["password"] + "\n"
                                    )
                                    tcpconn.send(
                                        "gps interval: "
                                        + (str(config["interval"]), "OFF")[
                                            config["interval"] <= 0
                                        ]
                                        + "\n"
                                    )
    ...
    if "\n" in accumulate:
        ...
        if params[0] == "@reset":
            ...
            if len(params) == 2:
                if params[1] == "battery":
                    print(
                        "BAT: " + str(inaData[1][0]) + "V " + str(inaData[1][1]) + "A"
                    )
                if params[1] == "3v3":
                    print(
                        "3V3: " + str(inaData[3][0]) + "V " + str(inaData[3][1]) + "A"
                    )
                if params[1] == "solar":
                    print(
                        "SOL: " + str(inaData[2][0]) + "V " + str(inaData[2][1]) + "A"
                    )
            else:
                ...
                print(
                    "gps interval: "
                    + (str(config["interval"]), "OFF")[config["interval"] <= 0]
                    + "\n"
                )
        elif params[0] == "@factory":
            microcontroller.nvm[0] = 0
...
def httpInit():
    ...
    def on_msg(request):
        ...
        d = {
            "i": 1,
            "t": urlDecode(qs["user_to"]),
            "f": urlDecode(qs["user_from"]),
            "s": urlDecode(qs["user_subject"]),
            "m": urlDecode(qs["user_message"]),
        }
        s = json.dumps(d)
...
def gpsInit():
    ...
    displayLine(
        3,
        "GPS Ping: "
        + (str(config["interval"]) + "min", "OFF")[config["interval"] <= 0],
    )
